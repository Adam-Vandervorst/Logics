...
def create_graph(formula: LTL): BA[String] =
  val ba = BA[String]()
  ...
  val us = formula.subformulas().collect{
    case u @ Until(p, q) =>
      ba.states.filter(n => !now(n).contains(u) || now(n).contains(q))
  }.toSet
  val finals = if us.nonEmpty then us else Set(ba.states)
