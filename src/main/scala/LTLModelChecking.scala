...
//  val ba = create_graph(X(X("p") U "q"))
//  val ba = create_graph(F(X(X("p") U "q")).nnf())
//  val ba = create_graph(Next(Var("p")))
//  val ba = create_graph(Or(Next(Var("p")), Next(Var("q"))))
//  val ba = create_graph(G("p" \/ X("q")).nnf())
//  val ba = create_graph(((False R (True U "p1")) /\ (True U ("q" /\ (False R !"r")))).nnf())
